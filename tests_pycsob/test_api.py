...
@freeze_time("2019-05-02 16:14:26")
class CsobClientBytesTests(TestCase):

    dttm = "20190502161426"
    dttime = datetime.datetime(2019, 5, 2, 16, 14, 26)

    def setUp(self):
        self.data = open(KEY_PATH, "r").read()
        self.c = CsobClient(merchant_id='MERCHANT',
                            base_url='https://gw.cz',
                            private_key_file=self.data,
                            csob_pub_key_file=self.data)

    @responses.activate
    def test_echo_post(self):
        resp_payload = utils.mk_payload(self.data, pairs=(
            ('dttm', utils.dttm()),
            ('resultCode', conf.RETURN_CODE_OK),
            ('resultMessage', 'OK'),
        ))
    ...
    @responses.activate
    def test_echo_get(self):
        payload = utils.mk_payload(self.data, pairs=(
            ('merchantId', self.c.merchant_id),
            ('dttm', utils.dttm()),
        ))
        resp_payload = utils.mk_payload(self.data, pairs=(
    ...
    def test_sign_message(self):
        ...
        assert payload['description'] == msg
        sig = payload.pop('signature')
        assert utils.verify(payload, sig, self.data)
    ...
    def test_payment_status_extension(self):
        ...
        ext_payload = utils.mk_payload(self.data, pairs=(
            ('extension', 'maskClnRP'),
            ('dttm', utils.dttm()),
            ('maskedCln', '****1234'),
            ('expiration', '12/20'),
